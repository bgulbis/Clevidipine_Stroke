...
tbl <- CreateTableOne(vars, "group", df[df$diagnosis == "stroke", ])
ptbl <- print(tbl, printToggle = FALSE)
knitr::kable(ptbl[, 1:3], caption = "Stroke")
```
...
tbl <- CreateTableOne(vars, "group", df[df$diagnosis == "ich", ])
ptbl <- print(tbl, printToggle = FALSE)
...
sbp %>%
    ...
    geom_point(alpha = 0.2, shape = 1, size = 0.5) +
    geom_smooth(aes(color = group)) +
    facet_grid(. ~ diagnosis) +
    scale_color_brewer(palette = "Set1", labels = c("Clevidipine", "Nicardipine"), guide = guide_legend(title = NULL)) 
```
